  ...
  return (
    <div>
      ...
      <div className="space-y-2">
        {levels.map(level => (
          <div key={level.id} className="bg-slate-800/50 rounded-lg p-3 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
            <div className="flex-1 min-w-0"><span className="font-bold">{level.level_name}</span> – {level.school_name} ({level.start_year}-{level.end_year}) <span className="text-xs bg-slate-700 px-2 py-0.5 rounded">{level.status}</span></div>
            <div className="flex gap-2">
